...
export function parseJobList(jobData) {
  ...
  return jobs.map((job) => {
    const position =
      job.querySelector(".base-search-card__title")?.textContent.trim() || "";
    const company =
      job.querySelector(".base-search-card__subtitle")?.textContent.trim() ||
      "";
    const location =
      job.querySelector(".job-search-card__location")?.textContent.trim() || "";
    ...
    const salary =
      job
        ...
        .replaceAll(" ", "") || "";
    const jobUrl =
      job.querySelector(".base-card__full-link")?.getAttribute("href") || "";
